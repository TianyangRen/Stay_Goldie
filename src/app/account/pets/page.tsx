...
export default async function AccountPetsPage() {
  ...
  const cards = pets.map((pet) => ({
    id: pet.id,
    name: pet.name,
    breedLine: `${pet.breed ?? "Breed not set"} · ${pet.sizeTier ?? "Size not set"}`,
    ...
    <section className="section-surface py-14">
      <div className="section-wrap">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <Reveal>
            <div>
              <h1 className="text-3xl font-semibold text-[var(--sg-green)]">My pets</h1>
              <p className="mt-2 text-sm text-zinc-600">
                Manage every pup under one login—profiles power quotes, boarding, and health notes.
              </p>
            </div>
          </Reveal>
          <Reveal delay={0.06}>
        ...
        {pets.length === 0 ? (
          <Reveal>
            <div className="mt-10 rounded-3xl border border-dashed border-[var(--sg-border-subtle)] bg-[var(--sg-surface)] p-10 text-center">
              <p className="text-sm text-zinc-600">You have not added a pet profile yet.</p>
